...
if request.method=="POST":
    ...
    if form.is_valid():
        try:
            ...
            homeworks.save()
            messages.success(request,f'Homeework Added from {request.user.username}!!')
else:
    form=Homeworkform()
...
def todo (request):
    if request.method=='POST':
        ...
        form =TodoForm()
    todo=Todo.objects.filter(user=requst.user)
    if len(todo)==0:
        todos_done=True
    else:
        todos_done=False
...
def dictionary(request):
     if request.method=="POST":
        ...
        try:
            ...
            synonyms=answer[0]['meanings'][0]['defintions'][0]['synonyms']
            context={
                'form':form,
                'input':text,
                'phonetics':phonetics,
